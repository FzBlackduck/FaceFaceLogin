...
==============================================================================*/
package com.example.facelogin.tracking
...
import com.example.facelogin.env.BorderedText
import com.example.facelogin.env.ImageUtils.getTransformationMatrix
import com.example.facelogin.env.Logger
import com.example.facelogin.tflite.SimilarityClassifier.Recognition
import java.util.*
...
class MultiBoxTracker(context: Context) {
    ...
    fun draw(canvas: Canvas) {
        ...
        for (recognition in trackedObjects) {
            ...
            @SuppressLint("DefaultLocale") val strConfidence = if (recognition.detectionConfidence < 0) "" else String.format("%.2f", recognition.detectionConfidence) + ""
            val labelString = if (!TextUtils.isEmpty(recognition.title)) String.format("%s %s", recognition.title, strConfidence) else strConfidence
